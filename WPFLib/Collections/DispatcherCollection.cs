...
namespace WPFLib
{
    ...
    public class DispatcherCollection<T> : ObservableCollection<T>
    {
        ...
        public static ManualResetEvent Create(ref DispatcherCollection<T> var, ManualResetEvent resetEvent)
        {
            ...
            {
                collection = new DispatcherCollection<T>();
            }
            else
            {
        ...
        /// </summary>
        public DispatcherCollection(List<T> list)
            : base(list)
        {
            //_dispatcher = Dispatcher.FromThread(Thread.CurrentThread);
        }

        public DispatcherCollection(IEnumerable<T> list)
            : base(list)
        {
            //_dispatcher = Dispatcher.FromThread(Thread.CurrentThread);
        }
        ...
        protected override void ClearItems()
        {
            ...
            {
                _dispatcher.Invoke(_updatePriority, new ZeroDelegate(ClearItemsImpl));
            }
        }

        void ClearItemsImpl()
        {
            new List<T>(this).ForEach(t => base.Remove(t));
        }
