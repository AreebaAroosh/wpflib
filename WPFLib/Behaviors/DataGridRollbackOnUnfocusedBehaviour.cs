...
{
    /// <summary>
    /// Решает проблему с 'DeferRefresh' is not allowed during an AddNew or EditItem
    /// </summary>
    public class DataGridRollbackOnUnfocusedBehaviour
    {
        ...
        private static DataGrid GetParentDatagrid(UIElement element)
        {
            ...
            {
                ItemsPresenter parentItemsPresenter = VisualHelper.FindVisualAncestor<ItemsPresenter>((element as ComboBoxItem));
            ...
            DataGrid parentDatagrid = VisualHelper.FindVisualAncestor<DataGrid>(childElement); //let's see if the new focused element is inside a datagrid
